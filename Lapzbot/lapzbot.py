...
import discord
import datetime
from Lapzbot import redis_manager, events
from Lapzbot.Modules import commands_parser, ranking, osu


class Lapzbot(discord.Client):
    """
    Lapzbot, the multi-purpose bot for Discord.

    For more information about Lapzbot, visit out website
        http://lapzbot.xyz
    We are also present in the following social media sites:
        https://twitter.com/lapzbot
        https://www.facebook.com/lapzbot/

    If you want to contribute please take a look at the on-boarding
    page here: http://lapzbot.xyz/contribute

    """
    __author__ = "Jewel Mahanta"
    ...
    async def on_ready(self):
        ...
        :return:
        """
        self.redis.set("LAPZBOT_SENT_MESSAGES", 0)
        self.redis.set("LAPZBOT_LOG_WRITES", 0)
        self.redis.set("imgur_remain", "0")
        self.redis.set("imgur_limit", "0")
        self.redis.set("imgur_reset", "0")
        self.redis.set("lapzbot_remain", "0")
        self.redis.set("lapzbot_limit", "0")
    ...
    async def on_server_join(self, server):
        """
        Called when Lapzbot joins a server
    ...
    async def on_server_remove(self, server):
        """
        Called when Lapzbot is removed from a server
