...
async def about(ctx, message):
    """
    Change Lapzbot profile about_me text
...
async def profile(ctx, message):
    """
    Returns a link to your current Lapzbot profile
    ...
    cursor = db.user_db_new.find({"user_id": message.author.id})
    if not cursor.count() == 0:
        await ctx.send_message(message.channel, "http://lapzbot.xyz/u/{}".format(message.author.id))
        return
