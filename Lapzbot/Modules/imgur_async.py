...
"""
from Lapzbot import mongo_manager, redis_manager
import random
import json
import aiohttp
import yaml
...
class Imgur:
    """
    Lapzbot's Custom Imgur Async Library
    """
    ...
    async def request_imgur(self, subreddit):
        ...
        try:
            ...
            self.redis.set("lapzbot_remain", str(response.headers['X-RATELIMIT-USERREMAINING']))
            self.redis.set("lapzbot_limit", str(response.headers['X-RATELIMIT-USERLIMIT']))

            if int(self.redis.get("lapzbot_remain")) < 5:
